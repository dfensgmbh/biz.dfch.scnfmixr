...
class InitialiseIn2(StateBase):
    """External insert/effect IN2 detection.

    Detects the external input/output (insert IN2).
    """
    ...
    def __init__(self):
        ...
        super().__init__(
            info_enter=UiEventInfo(
                StateEvent.INITIALISE_IN2_ENTER, True),
            info_leave=None,
        )
    ...
    def on_enter(self, ctx: ExecutionContext) -> None:
        ...
        value = app_ctx.audio_device_map[AudioDevice.IN2]
        if SKIP_USB_PORT == value:
            log.info("Skipping device '%s' ...", AudioDevice.IN2.name)
            msg = SystemMessage.InputEvent(InitialiseIn2.Event.SKIP_DEVICE)
            ctx.events.publish_first(msg)
            return

        log.info("Enqueueing event: '%s' [%s].",
                 InitialiseIn2.Event.DETECT_DEVICE.name,
                 InitialiseIn2.Event.DETECT_DEVICE.value)

        msg = SystemMessage.InputEvent(InitialiseIn2.Event.DETECT_DEVICE)
        ctx.events.publish_first(msg)
